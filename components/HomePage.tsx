...
const NewsItem: React.FC<{date: string, title: string, tag: string, onClick?: () => void}> = ({date, title, tag, onClick}) => (
  <button className="flex items-center gap-4 p-3 rounded-lg hover:bg-white/5 transition-colors text-left w-full" onClick={onClick}>
...
export const HomePage: React.FC<HomePageProps> = ({ onSelectGame, onNavigate }) => {
  const navigate = (page: string) => {
    if (onNavigate) {
      onNavigate(page);
    } else {
      const evt = new CustomEvent('neoanno:navigate', { detail: { to: page } });
      ...
      <nav className="fixed top-0 w-full z-50 bg-[#0b0f19]/80 backdrop-blur-md border-b border-white/5">
          <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
              ...
              <div className="flex items-center gap-3">
                 <button onClick={() => navigate('settings')} className="px-3 py-1.5 rounded-lg bg-black/20 border border-white/5 hover:bg-black/30 text-slate-400 hover:text-white transition-all text-xs">Settings</button>
                 <button onClick={() => navigate('about')} className="px-3 py-1.5 rounded-lg bg-black/20 border border-white/5 hover:bg-black/30 text-slate-400 hover:text-white transition-all text-xs">About</button>
      ...
      <section id="features" className="py-24 max-w-7xl mx-auto px-6 grid lg:grid-cols-3 gap-12">
          ...
          <div id="updates" className="bg-[#0f172a] rounded-2xl border border-white/5 p-6 h-fit">
              ...
              <div className="space-y-1">
                  <NewsItem date="Jan 07" title="Added Anno 117 building data (290 buildings)" tag="Data" onClick={() => navigate('updates')} />
                  <NewsItem date="Jan 06" title="Integrated production chains, regions, DLCs" tag="Core" onClick={() => navigate('updates')} />
                  <NewsItem date="Jan 05" title="Genetic solver foundational work" tag="Core" onClick={() => navigate('updates')} />
              </div>
              ...
              <div className="mt-6 pt-6 border-t border-white/5">
                  ...
                  <div className="flex gap-3 flex-wrap">
                      <button onClick={() => navigate('tutorial')} className="px-3 py-1.5 rounded-lg bg-black/30 border border-white/10 hover:bg-black/40 text-slate-400 hover:text-white transition-all text-xs">Tutorial</button>
                      <button onClick={() => navigate('bug')} className="px-3 py-1.5 rounded-lg bg-black/30 border border-white/10 hover:bg-black/40 text-slate-400 hover:text-white transition-all text-xs">Report Bug</button>
                      <a href="https://github.com/benoakleigh4/NeoAnno-Designer" target="_blank" rel="noreferrer" className="px-3 py-1.5 rounded-lg bg-black/30 border border-white/10 hover:bg-black/40 text-slate-400 hover:text-white transition-all text-xs flex items-center gap-1.5">
